...
app.post("/register", async(req,res)=>{
  ...
  const user = await User.create({
    email,password:hashed,credits:0,role:"user",paid:false,
    purchaseHistory:[],searchHistory:[],searchesUsed:0
  });
...
/* ===================================================== */
/* SECURE ETSY SEARCH FIABLE 2026 */
/* ===================================================== */
...
app.post("/search-etsy", auth, async(req,res)=>{
  ...
  try{
    ...
    // 🔥 Extraction fiable
    const imgRegex = /"image_url":"(https:\\/\\/i\.etsystatic\.com[^"]+)"/g;
    const linkRegex = /"url":"(https:\\/\\/www\.etsy\.com\/listing\/\d+)"/g;
...
app.post("/analyze-images", auth, upload.array("images"), async(req,res)=>{
  ...
  for(const file of req.files){
    ...
    try{
      ...
      console.log("Image pipeline error",err.message);
    }
  }
  res.json({results});
});
